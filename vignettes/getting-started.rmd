...
```{r}
crosswalk(cog1 = "mmse", cog2 = "moca", data = cogsim, num_iter = 500) 
```
...
The unconditional method can also be used based only on summary statistics. (In many cases, it may be more statistically efficient to do so, as the number of unconditional split iterations must be chose with care.)
