...
The `crosswalk` element in the list returned contains an estimated mean, standard error, and ($1-\alpha$)% confidence interval (by default $\alpha=0.05$), accounting for uncertainty in both the crosswalk estimate and published summary.^[See [$f=AB$](https://en.wikipedia.org/wiki/Propagation_of_uncertainty).] In this case, we see the estimated difference in MoCA comparing APOE-ε4 carriers to non-carriers is -2.87 (, SE = 0.140, 95% CI: [-3.15, -2.60]), based on the information is `dcw_se`.

We are still working on building out this function and adding summary methods for more convenient review of the results, so the particulars may change. The help documentation for `do_crosswalk()` contains information on additional features of this function.
